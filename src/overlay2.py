...
out = cv2.VideoWriter("videos/result2.mp4", fourcc, fps, (width, height))
...
for frame_number in tqdm(range(frame_count - (frame_range * frame_gap)), desc="Processing Frames"):
    ...
    if frame_count % 10 == 0:
        x = get_feature(4, [frame_number], frame_range, frame_gap, False, True, True, False, True, False, False, False, False, True, True)
        if x is None:
            continue
