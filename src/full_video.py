...
# Helper functions
def crop_person(box, frame, player, start_frame, frame_number):
    if torch.equal(box, torch.zeros_like(box)):
        return
    
    x1, y1, x2, y2 = map(int, box[0].tolist())
    ...
    filename = f"{directory}/player_{player}.jpg"
    ...
    for key_frame, value_frame in loaded_keys.items():
        ...
        try:
            while cap.isOpened() and frame_number <= 2:
                ...
                crop_person(player_1_boxes, frame, "left", start_frame, frame_number)
                crop_person(player_2_boxes, frame, "right", start_frame, frame_number)
